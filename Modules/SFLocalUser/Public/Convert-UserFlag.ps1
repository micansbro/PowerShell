...
Convert User Flags
.DESCRIPTION
This function converts user flags into human readable properties.
The function outputs the properties into a single string object.
.EXAMPLE
$LocalUsers = Get-LocalUserAccount
Convert-UserFlag -userflag $users[4].userflags.value
.EXAMPLE
$LocalUsers = Get-LocalUserAccount
Convert-UserFlag -UserFlag $LocalUsers.Userflags.value
.NOTES
Taken from https://mcpmag.com/articles/2015/04/15/reporting-on-local-accounts.aspx
Version - 1.1
Date - 26/01/2017
